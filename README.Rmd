...
This package is designed to work with the __tidyverse__, harnessing the functions within __broom__, __tidyr__, __dplyr__ and __purrr__ to extract estimates and plot things easily with __ggplot2__.

### Installation and examples

#### 1. Installation

__nls.multstart__ can be installed from CRAN using __install.packages()__ or from GitHub using __remotes__. 
...
An alternative to the shotgun approach is to use **Latin Hypercube Sampling** (LHS), which can only be used when `iter` is set to a single number. Instead of sampling from a uniform distribution across the bounds of each parameter, these methods try to take a set of samples from the range of parameter values that covers the parameter space optimally for any given set of parameters. *This approach can result in less iterations being needed to get the same reliability of model fitting than either the shotgun or grid-start approaches*. You can specify different methods used for the LHS using `lhstype`. Options are "random", "improved", maximin", or "genetic". You can learn more about the options on the [lhs package website](https://bertcarnell.github.io/lhs/index.html).

```{r run_nls_mulstart_lhs}
# subset dataset
d_1 <- subset(Chlorella_TRC, curve_id == 1)
